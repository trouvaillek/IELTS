---
name: ielts-reading
description: 雅思阅读精读和错题分析。Use when the user asks to analyze IELTS reading passages/questions, explain wrong answers, extract paraphrases, practice T/F/NG, or says “分析阅读”, “这道为什么错”, “同义替换”, “阅读训练”. Writes reading submissions and synonym YAML under ~/.ielts/.
---
...
```bash
mkdir -p ~/.ielts/reading/{submissions,synonyms}
```
...
- `band`: 按算分表换算（13 题这一篇大约 `7-8 题→6.0`、`9-10 题→6.5-7.0`，全卷 40 题用标准换算）
- `question_types[].type`: `tfng | ynng | matching_info | matching_features | matching_headings | mcq | summary | sentence_completion | short_answer | table | flow_chart`
...
## 下一步
- 同类题型再做一篇 → 重点看 {具体题型}
- 看正确率趋势：`“打开雅思 dashboard”`
```

### Phase 6：写文件（V3 关键）

按上面 schema 写两个文件，告诉用户：
